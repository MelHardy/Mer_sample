...
cm_response$wrtest2 <- factor(cm_response$wrtest2, levels=c(-2,0,2), labels=c("loss", "draw", "win"))
freq(cm_response$wrtest2)

##Set up a third vector for a third endpoint which is tte
wrtest3 <- c()

##Create a loop that calculate the wins (3), losses (-3) and draws (0) for the third TTE endpoint 
##microbiological relapse
for (i in data$id[which(data$intervention == 1)]){
  for (j in data$id[which(data$intervention == 2)]){ 
    if((data$mortality_30d[data$id == i]==data$mortality_30d[data$id == j]) & (data$cm_response[data$id == i]==data$cm_response[data$id == j])){{
      if(data$days_microrelapse[data$id == i]>data$days_microrelapse[data$id == j]){
        wrtest3 <- c(wrtest3,3)}
      else if(data$days_microrelapse[data$id == i]<data$days_microrelapse[data$id == j]){
        wrtest3 <- c(wrtest3,-3)}
      else if(data$days_microrelapse[data$id == i]==data$days_microrelapse[data$id == j])
        wrtest3 <- c(wrtest3,0)}}
  }
...
tt_microrelapse <-data.frame(wrtest3)

tt_microrelapse$wrtest3 <- factor(tt_microrelapse$wrtest3, levels=c(-3,0,3), labels=c("loss", "draw", "win"))
freq(tt_microrelapse$wrtest3)
